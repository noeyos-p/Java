...
public class Mega {
    ...
    private static void adminResult() {
        ...
        for (int i = 0; i < MENU_COUNT; i++) {
            if (products[i] != null) { // (! (products[i] == null))
                String strMenu = (i + 1) + ". " + products[i] + " : " + sales[i] + "원  ";
            ...
            if (updateMenuNumber >= 1 && updateMenuNumber <= MENU_COUNT) {
                if (products[updateMenuNumber - 1] != null) {
                    ...
                    System.out.println("현재의 " + products[updateMenuNumber - 1] +
                            " 가격은 " + price[updateMenuNumber - 1] + "원 입니다.");
    ...
    private static void order() {
        while (true) {
            System.out.println("======================");
            viewMenu();
            System.out.println("주문 할 번호를 입력하세요.");
            ...
            if (orderNumber >= 1 && orderNumber <= 5) {
                ...
                if (products[orderNumber - 1] != null) {
                    System.out.println(products[orderNumber - 1] + " 주문이 완료되었습니다. 감사합니다.");
                    System.out.println("주문하신 " + products[orderNumber - 1] + "가 나왔습니다.");
                    // 판매 금액을 처리한 후 메인 메뉴로 돌아가야 함.
                    sales[orderNumber - 1] += price[orderNumber - 1];
                    return;
                } else {
    ...
    private static void viewMenu() {
        ...
        for (int i = 0; i < MENU_COUNT; i++) {
            if (products[i] != null) { // (! (products[i] == null))
                String strMenu = (i + 1) + ". " + products[i] + " : " + price[i] + "원  ";
                System.out.println(strMenu);
            }
        }
